...
public class RobotContainer {
  ...
  private final VisionDriveCommand m_visionDriveCommand = new VisionDriveCommand(m_driveSubsystem, m_controller,
      m_visionSubsystem);
  private final IndexerSubsystem m_indexerSubsystem = new IndexerSubsystem();

  private final Command m_tarmacAuton = new AutonDriveCommand(m_driveSubsystem, 0,
      Constants.AutonConstants.kautonSpeedBackwards).withTimeout(Constants.AutonConstants.kautonDriveTime);
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_controller, XboxController.Button.kA.value)
        .whenPressed(m_visionDriveCommand)
        .whenReleased(m_arcadeDriveCommand);

    JoystickButton m_DriverLBumper = new JoystickButton(m_controller, XboxController.Button.kLeftBumper.value);
    ...
    JoystickButton m_DriverRBumper = new JoystickButton(m_controller, XboxController.Button.kRightBumper.value);
        ...
        .setDefaultCommand(
            new RunCommand(
                () -> m_driveSubsystem.arcadeDrive(
                    m_controller.getRightTriggerAxis() - m_controller.getLeftTriggerAxis(), m_controller.getLeftX()),
                m_driveSubsystem));
  }
